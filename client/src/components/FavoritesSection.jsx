...
const FavoritesSection = ({ onFileSelect, onPathChange, refreshTrigger }) => {
  ...
  const loadFavorites = () => {
    ...
    const directories = favs.filter(f => f.type === 'directory');
    setFavorites(directories);
  ...
  const handleRemove = (e, dirPath) => {
    e.stopPropagation();
    favoritesService.remove(dirPath);
  ...
  return (
    <div className="px-4 sm:px-6 md:px-8 lg:px-12 py-6 sm:py-8 border-b border-white/10">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-white">Favorites</h2>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4">
          {favorites.map((dir, index) => {
